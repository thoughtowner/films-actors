...
#             select(FilmToActor).where(FilmToActor.film_id == object_id)
#         ).all()
#     elif object_model == Actor:
#         films = session.scalars(
#             select(FilmToActor).where(FilmToActor.actor_id == object_id)
#         ).all()
    ...
    def update_object(new_object_data: dict, session: Session):
        try:
            session.bulk_update_mappings(
                object_model, [{'id': new_object_data['id'], **new_object_data}])
            session.commit()
    ...
    try:
        query = select(Actor).select_from(
            FilmToActor.__table__.join(
                Actor, FilmToActor.actor_id == Actor.id)).where(
